  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const [
        ...
        reviewsRes, favoritesRes, roleRequestsRes, paymentsRes,
      ] = await Promise.allSettled([
        // ✅ Fix: Users ko limit 5 se fetch karo taaki recentUsers mil saken
        getUsers({ limit: 5 }), getGuiders({ limit: 1 }), getPhotographers({ limit: 1 }),
        getPlaces({ limit: 1 }), getBookings({ limit: 1 }), getReviews({ limit: 1 }),
        getFavorites({ limit: 1 }),
        // ✅ Fix: Role requests ko limit 5 se fetch karo
        getRoleRequests({ limit: 5 }), getPayments({ limit: 1 }),
      ]);
