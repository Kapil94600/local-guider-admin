...
// ✅ Helper: Image URL ko absolute banao
...
const RequestDetailsModal = ({ request, onClose }) => {
  ...
  const ImageCard = ({ label, url }) => {
    ...
    return (
      <Box sx={{ textAlign: 'center' }}>
        <Typography variant="caption" sx={{ color: COLORS.textMuted, display: 'block', mb: 1 }}>{label}</Typography>
        {imageUrl ? (
          <img src={imageUrl} alt={label} style={{ width: 100, height: 100, borderRadius: 8, objectFit: 'cover', border: '1px solid #eee' }} />
        ) : (
  ...
  return (
    <Dialog open={!!request} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ fontFamily: FONT_DISPLAY, fontWeight: 700 }}>Request Details</DialogTitle>
      <DialogContent dividers>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <Chip label={request.requestedRole} color={request.requestedRole === 'GUIDER' ? 'info' : 'secondary'} size="small" />
          </Grid>
          <Grid item xs={12} sm={6}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <Typography variant="body1">{request.createdAt ? new Date(request.createdAt).toLocaleDateString() : 'N/A'}</Typography>
          </Grid>

          <Grid item xs={12}>
            <Typography variant="subtitle2" color="textSecondary" sx={{ mb: 1 }}>Selected Places</Typography>
            {placeNames.length > 0 ? (
          ...
          <Grid item xs={12}>
            ...
            <Grid container spacing={2}>
              <Grid item xs={3}><ImageCard label="Selfie" url={request.selfieUrl} /></Grid>
              <Grid item xs={3}><ImageCard label="Profile Photo" url={request.profilePhotoUrl} /></Grid>
              <Grid item xs={3}><ImageCard label="ID Front" url={request.idFrontUrl} /></Grid>
              <Grid item xs={3}><ImageCard label="ID Back" url={request.idBackUrl} /></Grid>
            </Grid>
          </Grid>
          <Grid item xs={12}>
...
const RoleRequests = () => {
  ...
  const filtered = (items || []).filter((item) => {
    const matchesSearch = !searchTerm || item.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) || item.companyName?.toLowerCase().includes(searchTerm.toLowerCase()) || item.location?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  // ✅ UPDATED: rejectRoleRequest ab object leta hai ({ id, adminMessage })
  const handleReject = async (id) => {
    const adminMessage = window.prompt('Reason for rejection:');
    if (adminMessage === null) return; // user ne cancel kiya
    try {
  ...
  const columns = [
    ...
    { field: 'location', headerName: 'Location', flex: 0.8 },
    {
      ...
      renderCell: (p) => <Chip label={p.row.requestedRole} color={p.row.requestedRole === 'GUIDER' ? 'info' : 'secondary'} size="small" />,
    },
    {
      field: 'status', headerName: 'Status', flex: 0.7,
      renderCell: (p) => <Chip label={p.row.status} color={p.row.status === 'APPROVED' ? 'success' : p.row.status === 'REJECTED' ? 'error' : 'warning'} size="small" />,
    },
    {
    ...
    },
    {
      ...
      renderCell: (p) => (
        <Stack direction="row" spacing={0.5}>
          <IconButton onClick={() => setSelectedRequest(p.row)} sx={{ color: COLORS.sky }}><Visibility /></IconButton>
          {p.row.status === 'PENDING' && (
            <>
              <IconButton onClick={() => handleApprove(p.row.id)} sx={{ color: 'success.main' }}><CheckCircle /></IconButton>
              <IconButton onClick={() => handleReject(p.row.id)} sx={{ color: 'error.main' }}><Cancel /></IconButton>
            </>
          )}
        </Stack>
      ),
        ...
        <Card key={req.id} sx={{ borderRadius: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <CardContent>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
              ...
              <Chip label={req.requestedRole} size="small" color={req.requestedRole === 'GUIDER' ? 'info' : 'secondary'} />
            </Box>
            <Typography variant="body2" color="textSecondary" sx={{ mb: 0.5 }}>{req.companyName || 'N/A'} • {req.location || 'N/A'}</Typography>
            ...
            <Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ borderTop: '1px solid #f1f5f9', pt: 1 }}>
              <IconButton size="small" onClick={() => setSelectedRequest(req)} sx={{ color: COLORS.sky }}><Visibility fontSize="small" /></IconButton>
              {req.status === 'PENDING' && (
                <>
                  <IconButton size="small" onClick={() => handleApprove(req.id)} sx={{ color: 'success.main' }}><CheckCircle fontSize="small" /></IconButton>
                  <IconButton size="small" onClick={() => handleReject(req.id)} sx={{ color: 'error.main' }}><Cancel fontSize="small" /></IconButton>
                </>
              )}
            </Stack>
          </CardContent>
    ...
    <Box className="fade-in" sx={{ p: { xs: 2, md: 3 }, mt: 0, pt: 1 }}>
      ...
      {isMobile ? (
        ...
        <Paper elevation={0} sx={{ p: 2, borderRadius: 4, border: '1px solid #e2e8f0', bgcolor: 'white' }}>
          {loading ? <Loader /> : (
            <DataGrid rows={filtered} columns={columns} pageSize={pagination.limit} rowsPerPageOptions={[5, 10, 25]}
