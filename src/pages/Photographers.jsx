...
const getEmail = (photographer) => {
  return photographer.user?.email || photographer.email || '—';
};

const getPhone = (photographer) => {
  return photographer.user?.phone || photographer.phone || '—';
};
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <Avatar
          src={getImageUrl(params.row.profileImage || params.row.user?.profileImage)}
    ...
    },
    {
      ...
      flex: 1,
      minWidth: 120,
      renderCell: (params) => params.row.cameraDetails || 'N/A',
    },
    {
      field: 'isActive',
      headerName: 'Status',
        ...
        <Card key={photographer.id} sx={{ borderRadius: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <CardContent>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
              <Avatar
                src={getImageUrl(photographer.profileImage || photographer.user?.profileImage)}
                alt={getFullName(photographer)}
              />
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography variant="subtitle1" fontWeight={600} noWrap>
                  {getFullName(photographer)}
                </Typography>
